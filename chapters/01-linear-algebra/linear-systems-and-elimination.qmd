...
1. 什么是线性方程、线性方程组、解与解集；
2. 怎样在方程组、矩阵方程和增广矩阵之间切换；
3. 什么叫两个方程组等价；
4. 为什么三类初等行变换不会改变解集；
5. 初等行变换怎样对应左乘初等矩阵；
6. 什么是主元、行阶梯形、简化行阶梯形、主元变量和自由变量；
7. Gaussian 消元与 Gauss–Jordan 消元有什么区别；
8. 怎样从阶梯形判断无解、唯一解或无穷多解；
9. 齐次方程组与非齐次方程组的全部解有什么关系；
10. 为什么手算消元与浮点数值求解必须区分；
11. 消元留下了哪些需要向量空间、基、维数与秩才能正式回答的问题。
...
竖线只用于提醒我“左边是未知量系数，右边是常数项”，不是一种新的矩阵运算。增广列也不是第 $n+1$ 个未知量。
...
Gaussian 消元通常先得到 REF，再通过回代求解；Gauss–Jordan 消元则继续消去主元上方元素，直到得到 RREF。同一个矩阵可以经不同路线得到不同 REF；RREF 则是唯一的。RREF 唯一性的证明不影响当前求解流程，我先记录结论，等后面整理行等价与子空间结构时再补证明。
...
这已经是完整的**消元判别准则**：把增广矩阵化为阶梯形，就能根据矛盾行与主元位置判断解的情况。但它仍是算法形式的判别。后面定义行列式、子式和秩后，我会把它压缩成更结构化的判别准则，并证明两种说法等价。
...
这里可以直观地说“非齐次解集是齐次解集的一次平移”。等定义子空间和仿射空间后，我会再给出正式术语。
...
我同时用 Strang 的行图景、列图景和方程组视角核对矩阵解释 [@strang2023linear]，并参考 Boyd 与 Vandenberghe 对应用线性方程、数值计算和最小二乘边界的组织 [@boyd2018applied]。
...
Gaussian 消元已经给出解的三种情形和一个可执行判别准则，但计算过程还留下了更具体的问题。

对一个 $n\times n$ 方阵，唯一解意味着每一列最终都出现主元。消元会把矩阵变成上三角形，此时主元位于对角线上。于是我自然想找一个标量，它能够：
...
行列式只能直接判别方阵是否具有唯一解；当行列式为零时，它不能单独区分“无解”和“无穷多解”，长方形系统也没有同样的行列式。这会进一步迫使我引入子式和秩。等一般方程组的有解判据、自由变量数量和齐次解结构都写清楚后，再进入向量空间、基与维数会更自然。
