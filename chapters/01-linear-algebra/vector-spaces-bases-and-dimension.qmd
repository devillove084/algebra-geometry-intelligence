...
这与上一页“主元个数”或“最大非零子式阶数”的定义相等。做行化简时，主元列标出哪些列提供了新方向；**原矩阵中**对应的主元列构成列空间的一组基。不能把化简后矩阵的列直接当作原列空间的基，因为行变换会改变列向量所在的具体空间位置。
...
![输入空间 $\mathbb{R}^n$ 中橙色零空间消耗 $n-r$ 维；输出空间 $\mathbb{R}^m$ 中绿色列空间提供 $r$ 个可达方向；秩-零度定理保证两端的自由度之和恰为 $n$。](../../images/linear-algebra/rank-nullity-spaces.svg){#fig-rank-nullity-spaces fig-alt="左框是输入空间 Rn，内含橙色的零空间和补集；中间箭头标注 A 映射；右框是输出空间 Rm，内含绿色列空间；底部公式 rank A 加 dim 零空间等于 n。" width="92%"}
...
若 $\mathbf{W}$ 真正低秩，输出会被限制在一个更低维的子空间。这是低秩分解和一些参数高效微调方法能够压缩线性更新的代数前提。对于深度学习框架中常见的带偏置层 $\mathbf{y}=\mathbf{W}\mathbf{x}+\mathbf{b}$，输出属于偏移后的仿射子空间 $\mathbf{b}+\mathcal{C}(\mathbf{W})$；此时独立方向数仍由 $\operatorname{rank}(\mathbf{W})$ 决定，只是整个输出集合被平移到另一个位置。

但“某层的激活在少数方向上方差较大”不自动等于“该层严格位于低维子空间”。前者是近似、数据相关且需要阈值的统计陈述；后者是精确的代数陈述。诊断表示时应报告奇异值、误差阈值、数据分布与任务变化，而不是只报告一个“有效维数”。
...
它至多有 $\min(N,d)$ 个维度，且会随数据集选择改变。这个张成空间描述“这批样本在线性意义下访问了哪些方向”，并不直接给出语义类别、因果机制或整个模型所有可能状态的结构。

# 用 NumPy 核对张成、坐标与零空间关系
...
print("rank(A) =", rank)
print("一个零空间方向：", null_vector)
print("x 在非标准基下的坐标：", coordinate)
```

`np.linalg.matrix_rank` 基于浮点奇异值阈值给出数值秩；在接近相关的真实数据上，改变阈值可能改变结果。本页的整数例子有精确关系，因此不会暴露这种数值歧义。
...
下一步需要在向量空间上加入额外结构：怎样定义长度、夹角和正交性？内积、范数与投影会回答这些问题，并把“一个子空间中的最佳近似”连接到最小二乘、attention 相似度和表示比较 [@strang2023linear; @axler2015linear; @boyd2018applied]。
