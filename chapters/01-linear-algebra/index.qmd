...
8. 子式、秩与一般方程组判别准则；
9. 向量空间、子空间、基与维数；
10. 交换引理、基扩充、维数定理与秩—零度；
11. 线性映射的坐标矩阵、换基与相似；
12. 行列式乘法性、余子式与相似不变量；
13. LU/PLU 分解、条件数与稳定求解；
14. 内积、范数、角度、正交补与基本子空间；
15. 正交基、投影、Gram--Schmidt 与 QR 分解；
16. 最小二乘、残差几何与数值求解；
17. 特征值、特征向量、对角化与不变子空间；
18. 谱定理、二次型、正定性与 Cholesky 分解；
19. 奇异值分解、伪逆、低秩近似与 PCA；
20. 数值特征值、数值秩与非正规矩阵敏感性。
...
整理几何直觉时，我主要参考 3Blue1Brown 对空间变换和基向量的可视化 [@sanderson2016essence]；整理列组合、方程组和子空间之间的联系时，我会对照 Strang [@strang2023linear]。证明部分会交叉查看丘维声老师的高等代数课程和 Axler 的线性映射视角 [@qiuAdvancedAlgebraCourse; @axler2015linear]；目前 B 站链接只作为查找第三方课程录像的入口，不当作官方版本。应用与数值计算部分参考 Boyd 与 Vandenberghe [@boyd2018applied]，机器学习例子则继续对照经典模式识别和 Transformer 原始论文 [@bishop2006pattern; @vaswani2017attention]。

这些资料帮助我核对概念和补足视角，但不会决定笔记顺序。我会按自己的知识缺口组织内容：先留下能工作的直觉、类型和例子，再补证明、抽象结构与数值边界。
