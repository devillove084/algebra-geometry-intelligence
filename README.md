...
| 状态 | 内容 |
|---|---|
| 已整理 | 数学语言导论；数、集合与区间；函数与映射；命题、量词与证明；微积分核心定义；概率统计核心定义；标量与向量；矩阵与线性映射；矩阵乘法与线性映射复合；线性方程组与 Gaussian 消元；排列、逆序对、逆序数与行列式；可逆性、Cramer 法则、子式、秩与一般方程组判别准则；向量空间、子空间、张成、线性相关、基、维数与秩—零度定理 |
| 整理中 | 内积、范数、正交、投影与最小二乘 |
| 之后补充 | 特征值与 SVD；系统性的微积分、概率论与统计推断 |
